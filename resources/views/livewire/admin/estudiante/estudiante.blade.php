<div class="w-full  mx-auto">
  ...
  <div x-data="{ open:false }" class="relative">
    ...
    <div x-data x-cloak class="relative">
      <!-- Select de búsqueda -->
      <x-searchable-select label="Estudiante" placeholder="--Selecciona al estudiante--" wire:model.live="query" {{--
        query ahora es el ID del alumno --}}>
    ...
    @if ($selectedAlumno)
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div class="flex items-center gap-4">
            @if (!empty($selectedAlumno['foto']))
              <img src="{{ asset('storage/estudiantes/' . $selectedAlumno['foto']) }}" alt="Foto del alumno"
      ...
      <div
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="font-semibold uppercase tracking-wide text-[11px] text-neutral-500 dark:text-neutral-400">Resumen
            académico</span>
        </div>
              ...
              <ul class="space-y-3">
                ...
                <li x-data="{show:false}" x-effect="document.body.style.overflow = show ? 'hidden':'auto'">
                  <div
                    ...
                    <button type="button" @click="show=true" :disabled="{{ $curpNombre ? 'false' : 'true' }}"
                ...
                <li x-data="{show:false}" x-effect="document.body.style.overflow = show ? 'hidden':'auto'">
                  <div
                    class="flex items-center gap-3 p-3 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-700/40 hover:bg-neutral-50 dark:hover:bg-neutral-700 transition">
                    <button type="button" @click="show=true" :disabled="{{ $actaNombre ? 'false' : 'true' }}"
                      class="disabled:opacity-50 disabled:cursor-not-allowed"
                      title="{{ $actaNombre ? 'Ver Acta' : 'Sin archivo' }}">
                      <img src="{{ asset('storage/acta_nacimiento.png') }}" class="h-8 w-8" alt="Acta de nacimiento">
                    </button>
                    <span class="flex-1 text-sm font-medium text-neutral-800 dark:text-neutral-100">ACTA DE
                      NACIMIENTO</span>
                    @if ($actaNombre)
                      <flux:badge color="green">Entregado</flux:badge>
                    @else
                  ...
                  <div x-cloak x-show="show" @keydown.escape.window="show=false" @click.self="show=false"
                    ...
                    <div
                      ...
                      @else
                        <div class="w-full h-full flex items-center justify-center p-6">
                          <p class="text-neutral-600 dark:text-neutral-300">No hay Acta de nacimiento.</p>
                        </div>
                      @endif
                    </div>
                  </div>
                ...
                <li x-data="{show:false}" x-effect="document.body.style.overflow = show ? 'hidden':'auto'">
                  <div
                    class="flex items-center gap-3 p-3 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-700/40 hover:bg-neutral-50 dark:hover:bg-neutral-700 transition">
                    <button type="button" @click="show=true" :disabled="{{ $certEstNombre ? 'false' : 'true' }}"
                      class="disabled:opacity-50 disabled:cursor-not-allowed"
                      title="{{ $certEstNombre ? 'Ver certificado' : 'Sin archivo' }}">
                      <img src="{{ asset('storage/certificado_estudios.png') }}" class="h-8 w-8"
                        alt="Certificado de estudios">
                    </button>
                    <span class="flex-1 text-sm font-medium text-neutral-800 dark:text-neutral-100">CERTIFICADO DE
                      ESTUDIOS</span>
                    @if ($certEstNombre)
                      <flux:badge color="green">Entregado</flux:badge>
                    @else
                  ...
                  <div x-cloak x-show="show" @keydown.escape.window="show=false" @click.self="show=false"
                    ...
                    <div
                      ...
                      @else
                        <div class="w-full h-full flex items-center justify-center p-6">
                          <p class="text-neutral-600 dark:text-neutral-300">No hay Certificado de estudios.</p>
                        </div>
                      @endif
                    </div>
                  </div>
                ...
                <li x-data="{show:false}" x-effect="document.body.style.overflow = show ? 'hidden':'auto'">
                  <div
                    class="flex items-center gap-3 p-3 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-700/40 hover:bg-neutral-50 dark:hover:bg-neutral-700 transition">
                    <button type="button" @click="show=true" :disabled="{{ $compDomNombre ? 'false' : 'true' }}"
                      class="disabled:opacity-50 disabled:cursor-not-allowed"
                      title="{{ $compDomNombre ? 'Ver comprobante' : 'Sin archivo' }}">
                      <img src="{{ asset('storage/comprobante_domicilio.png') }}" class="h-8 w-8"
                        alt="Comprobante de domicilio">
                    </button>
                    <span class="flex-1 text-sm font-medium text-neutral-800 dark:text-neutral-100">COMPROBANTE DE
                      DOMICILIO</span>
                    @if ($compDomNombre)
                      <flux:badge color="green">Entregado</flux:badge>
                    @else
                  ...
                  <div x-cloak x-show="show" @keydown.escape.window="show=false" @click.self="show=false"
                    ...
                    <div
                      ...
                      @else
                        <div class="w-full h-full flex items-center justify-center p-6">
                          <p class="text-neutral-600 dark:text-neutral-300">No hay Comprobante de domicilio.</p>
                        </div>
                      @endif
                    </div>
                  </div>
                ...
                <li x-data="{show:false}" x-effect="document.body.style.overflow = show ? 'hidden':'auto'">
                  <div
                    class="flex items-center gap-3 p-3 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-700/40 hover:bg-neutral-50 dark:hover:bg-neutral-700 transition">
                    <button type="button" @click="show=true" :disabled="{{ $certMedNombre ? 'false' : 'true' }}"
                      class="disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    <span class="flex-1 text-sm font-medium text-neutral-800 dark:text-neutral-100">CERTIFICADO
                      MÉDICO</span>
                    @if ($certMedNombre)
                      <flux:badge color="green">Entregado</flux:badge>
                    @else
                  ...
                  <div x-cloak x-show="show" @keydown.escape.window="show=false" @click.self="show=false"
                    ...
                    <div
                      ...
                      @else
                        <div class="w-full h-full flex items-center justify-center p-6">
                          <p class="text-neutral-600 dark:text-neutral-300">No hay Certificado médico.</p>
                        </div>
                      @endif
                    </div>
                  </div>
                ...
                <li x-data="{show:false}" x-effect="document.body.style.overflow = show ? 'hidden':'auto'">
                  <div
                    class="flex items-center gap-3 p-3 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-700/40 hover:bg-neutral-50 dark:hover:bg-neutral-700 transition">
                    <button type="button" @click="show=true" :disabled="{{ $ineNombre ? 'false' : 'true' }}"
                      class="disabled:opacity-50 disabled:cursor-not-allowed"
