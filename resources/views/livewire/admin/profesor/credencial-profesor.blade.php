...
<hr class="my-6">
    ...
    <div class="flex flex-col gap-2">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Credencial del profesor como estudiante</h1>
    </div>

     <form action="{{ route('admin.pdf.documentacion.credencial_profesor_estudiante') }}" method="GET" target="_blank" class="mt-4">
    ...
    @input="open = true"
    @blur="setTimeout(() => open = false, 150)"
    wire:keydown.arrow-down="selectIndexProfesorEstudianteDown"
...
@if (!empty($profesores_estudiante))
    <ul
        ...
        class="absolute w-full bg-white border mt-1 rounded shadow z-10 max-h-60 overflow-auto"
    >
        @foreach ($profesores_estudiante as $index => $profesor)
            <li
                class="p-2 cursor-pointer {{ $selectedIndexProfesorEstudiante === $index ? 'bg-blue-200' : '' }}"
                wire:click="selectProfesorEstudiante({{ $index }})"
            >
                <p class="font-bold text-indigo-600">
                    {{ $profesor['nombre'] ?? '' }} {{ $profesor['apellido_materno'] ?? '' }} {{ $profesor['apellido_paterno'] ?? '' }}
                </p>
...
@if (!empty($profesoresEstudianteSeleccionados))
    <div class="mt-4 p-4 border rounded bg-gray-50 dark:bg-gray-800 dark:text-white">
        <p class="font-bold mb-2">Profesores seleccionados:</p>
        <ul class="space-y-2">
            @foreach ($profesoresEstudianteSeleccionados as $profesor)
                <li class="flex items-center justify-between">
                    <div>
                        {{ $profesor['apellido_paterno'] ?? '' }} {{ $profesor['apellido_materno'] ?? '' }} {{ $profesor['nombre'] ?? '' }}
                        <span class="text-sm text-gray-600">({{ $profesor['user']['CURP'] ?? '' }})</span>
                    </div>
                    <button type="button" wire:click="eliminarProfesorEstudianteSeleccionado({{ $profesor['id'] }})" class="text-red-600 hover:text-red-800">✕</button>
                    <input type="hidden" name="profesores_ids[]" value="{{ $profesor['id'] }}">
                </li>
            @endforeach
        </ul>
