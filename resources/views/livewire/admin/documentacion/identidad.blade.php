<div x-data="{ open: false }" class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-end">
        <div class="flex flex-col sm:flex-row gap-4 flex-1">
            <!-- Licenciatura -->
            <div class="flex-1">
                <flux:select label="Licenciatura" placeholder="Seleccionar licenciatura"
                    wire:model.live="selectedLicenciatura" class="w-full">
        ...
        <div class="flex-shrink-0">
            <a href="{{ route('admin.pdf.documentacion.alumnos.documentacion', [
    ...
    <!-- Card contenedor -->
    <div
        class="relative  rounded-2xl border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow">
        <!-- Top Accent -->
        ...
        <div class="p-4 sm:p-6">
            <div class="relative max-w-3xl">
                ...
                <flux:input id="buscar-alumno" required label="Buscar alumno" wire:model.live.debounce.500ms="query"
                    name="alumno_id" type="text" placeholder="Buscar alumno por nombre, matrícula o CURP…"
                    autocomplete="off" @focus="open = true" @input="open = true"
                    @blur="setTimeout(() => open = false, 150)" wire:keydown.arrow-down="selectIndexDown"
                    wire:keydown.arrow-up="selectIndexUp" wire:keydown.enter="selectAlumno({{ $selectedIndex }})"
                    class="w-full" />

                <!-- Resultados -->
                @if (!empty($alumnos))
                    <ul x-show="open" x-transition x-cloak role="listbox"
                        ...
                        @forelse ($alumnos as $index => $alumno)
                            <li class="p-2 cursor-pointer hover:bg-neutral-50 dark:hover:bg-neutral-800 {{ $selectedIndex === $index ? 'bg-indigo-50 dark:bg-neutral-800' : '' }}"
                                wire:click="selectAlumno({{ $index }})" @mouseenter="open = true" role="option">
                                <p class="font-semibold text-indigo-700 dark:text-indigo-300 truncate">
                                    {{ $alumno['apellido_paterno'] ?? '' }} {{ $alumno['apellido_materno'] ?? '' }}
                                    {{ $alumno['nombre'] ?? '' }}
                                </p>
                                <p class="text-sm text-neutral-600 dark:text-neutral-400">
                                    Matrícula: <span class="font-mono">{{ $alumno['matricula'] ?? '' }}</span> · CURP:
                                    <span class="font-mono">{{ $alumno['CURP'] ?? '' }}</span>
                                </p>
                            </li>
                        @empty
                            <li class="p-3 text-sm text-neutral-600 dark:text-neutral-300">No se encontraron alumnos.
                            </li>
                        @endforelse
                    </ul>
                @endif
        ...
        <div wire:loading.delay wire:target="selectAlumno" class="px-4 sm:px-6 pb-4">
            <div
                class="flex items-center gap-2 rounded-xl border border-dashed border-gray-300 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800/50 px-4 py-3">
                <svg class="h-5 w-5 animate-spin text-indigo-600 dark:text-indigo-400" viewBox="0 0 24 24"
                    fill="none">
        ...
        @if ($selectedAlumno)
            <div class="px-4 sm:px-6 pb-6 space-y-4">
                <!-- Tarjeta alumno -->
                <div
                    class="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/60 p-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                        <p class="font-semibold text-neutral-900 dark:text-white">
                            {{ $selectedAlumno['apellido_paterno'] ?? '' }}
                            {{ $selectedAlumno['apellido_materno'] ?? '' }} {{ $selectedAlumno['nombre'] ?? '' }}
                        </p>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <a target="_blank"
                        ...
                        <svg aria-hidden="true" class="h-5 w-5" viewBox="0 0 22 31" fill="none"
                ...
                <div class="pt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <livewire:admin.documentacion.carga-documentos :key="$selectedAlumno['id'] . '_curp'" label="CURP"
                        wireId="CURP_documento" rutaGuardado="documentos/curp" :inscripcion-id="$selectedAlumno['id']" />

                    <livewire:admin.documentacion.carga-documentos :key="$selectedAlumno['id'] . '_acta_nacimiento'" label="Acta de Nacimiento"
                        wireId="acta_nacimiento" rutaGuardado="documentos/actas" :inscripcion-id="$selectedAlumno['id']" />

                    <livewire:admin.documentacion.carga-documentos :key="$selectedAlumno['id'] . '_certificado_estudios'" label="Certificado de Estudios"
                        wireId="certificado_estudios" rutaGuardado="documentos/certificado_estudios"
                        :inscripcion-id="$selectedAlumno['id']" />

                    <livewire:admin.documentacion.carga-documentos :key="$selectedAlumno['id'] . '_comprobante_domicilio'" label="Comprobante de Domicilio"
                        wireId="comprobante_domicilio" rutaGuardado="documentos/comprobante_domicilio"
                        :inscripcion-id="$selectedAlumno['id']" />

                    <livewire:admin.documentacion.carga-documentos :key="$selectedAlumno['id'] . '_certificado_medico'" label="Certificado Médico"
                        wireId="certificado_medico" rutaGuardado="documentos/certificado_medico" :inscripcion-id="$selectedAlumno['id']" />

                    <livewire:admin.documentacion.carga-documentos :key="$selectedAlumno['id'] . '_ine'" label="INE" wireId="ine"
                        rutaGuardado="documentos/ine" :inscripcion-id="$selectedAlumno['id']" />
                </div>
            </div>
        @endif
    </div>
