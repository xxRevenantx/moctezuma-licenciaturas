...
<body>
    ...
    <div style="text-align: center;">
        ...
        @if ($materia->licenciatura->imagen)
            <img class="img2" src="{{ public_path('storage/licenciaturas/' . $materia->licenciatura->imagen) }}"
                alt="Logo Licenciatura" height="100px" width="100">
        @else
            <img class="img2" src="{{ public_path('storage/logo-moctezuma.jpg') }}" alt="Logo" height="100px"
                width="100">
        @endif


    </div>
    <p style="font-size: 24px; text-align:center; font-weight:bold; margin-top:-30px; line-height:20px">CONCENTRADO DE
        CALIFICACIONES <br> CUATRIMESTRAL</p>
    <p style="font-size: 20px; color:#5b5b5b; text-align:center; font-weight:bold; margin-top:-25px; line-height:20px">
        CICLO ESCOLAR: {{ $ciclo_escolar->ciclo_escolar }}</p>

    {{-- AQUI VA LA TABLA --}}



    <table style="width: 100%; border-collapse: collapse;  font-size: 14px; margin-bottom: 0px;" border="1">
        <thead>
            <tr style="background-color: #f2f2f2; text-align: center;">
                <th colspan="3">CONCENTRADO DE CALIFICACIONES</th>
                <th colspan="5" style="text-transform: uppercase">FECHAS:
                    {{ \Carbon\Carbon::parse($periodos->inicio_periodo)->translatedFormat('d \d\e F \d\e\l Y') }} al
        ...
        <tbody>
            ...
            <tr>
                ...
                <td colspan="3">{{ $materia->profesor->nombre }} {{ $materia->profesor->apellido_paterno }}
                    {{ $materia->profesor->apellido_materno }}</td>
        ...
        <tbody>
            ...
            @foreach ($alumnos as $key => $alumno)
                <tr>
                    ...
                    <td style="text-align: left;">{{ $alumno->nombre }} {{ $alumno->apellido_paterno }}
    ...
    <p style="text-align:center; font-size: 14px; font-weight: bold; margin-top: 40px;">
    ...
    <footer>
        <p>{{ $escuela->nombre }} C.C.T. {{ $escuela->CCT }}. C. {{ $escuela->calle }} No.
            {{ $escuela->no_exterior }}, Col. {{ $escuela->colonia }}, C.P. {{ $escuela->codigo_postal }}, Cd.
            {{ $escuela->ciudad }}, {{ $escuela->estado }}.</p>
