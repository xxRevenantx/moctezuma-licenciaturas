...
class Inscripcion extends Component
{
    ...
    /**
     * Hook específico: cuando cambia generacion_id
     * Limpia cuatrimestre y carga los cuatrimestres de la generación.
     */
    public function updatedGeneracionId($value)
        {
            // Limpia antes de cargar
            $this->reset('cuatrimestre_id', 'cuatrimestres');

            if (!$value) {
                return;
            }

            // Traer SOLO el último periodo (por id más alto)
            $ultimo = Periodo::where('generacion_id', $value)
                ->orderByDesc('id')
                ->first();

            // Mantener la estructura que usa el @foreach (lista con 1 ítem)
            $this->cuatrimestres   = $ultimo ? [$ultimo] : [];

            // (Opcional) Autoseleccionar el cuatrimestre encontrado
            $this->cuatrimestre_id = $ultimo?->cuatrimestre_id;
        }
    ...
    public function updated($propertyName)
    {
        ...
        // Importante: el manejo de generacion_id se movió al hook updatedGeneracionId()
    }
