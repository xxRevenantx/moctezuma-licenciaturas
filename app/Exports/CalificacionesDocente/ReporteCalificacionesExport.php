...
class ReporteCalificacionesExport implements FromArray, WithStyles, WithEvents, ShouldAutoSize
{
    ...
    public function array(): array
    {
        ...
        $rows = [
            ...
            ['', '', '', '', ''],
            ['No.', 'ID de control interno', 'Matrícula SEG', 'Alumno', 'Calificación'],
        ];
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->freezePane('A7');
                $sheet->getStyle("A6:E{$ultimaAlumno}")->getBorders()->getAllBorders()->setBorderStyle('thin');
                $sheet->getStyle("A2:E4")->getFont()->setSize(10);
                $sheet->getStyle("A{$firmaRow}:E{$firmaRow}")->getBorders()->getTop()->setBorderStyle('thin');
                $sheet->getStyle("A{$firmaRow}:E{$firmaRow}")->getFont()->setBold(true);
            },
        ];
    }
}
